...
async function router(request, env) {
  ...
  if (method === 'GET' && userMatch) {
    const user = await env.DB.prepare('SELECT id, email, name, role, org_name, bio, location, verified, created_at FROM users WHERE id = ?').bind(userMatch[1]).first();
  ...
  if (method === 'POST' && path === '/api/users') {
    ...
    const user = await env.DB.prepare('SELECT id, email, name, role, org_name, bio, location, verified, created_at FROM users WHERE id = ?').bind(id).first();
